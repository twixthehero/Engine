...
namespace VoxEngine
{
	...
	void Input::Update()
	{
		...
		for (int i = 0; i < NUM_KEYS; i++)
			_currentKeys[_keys[i]] = _window->GetKey(_keys[i]) == GLFW_PRESS;

		for (int i = 0; i < NUM_BUTTONS; i++)
			_previousMouse[i] = _currentMouse[i];

		for (int i = 0; i < NUM_BUTTONS; i++)
			_currentMouse[i] = _window->GetMouse(i) == GLFW_PRESS;
	}
	...
	bool Input::GetMouse(unsigned int button)
	{
		return _currentMouse[button];
	}

	bool Input::GetMouseDown(unsigned int button)
	{
		return _currentMouse[button] && !_previousMouse[button];
	}

	bool Input::GetMouseUp(unsigned int button)
	{
		return !_currentMouse[button] && _previousMouse[button];
	}
